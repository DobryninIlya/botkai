...
def info():
    ...
    cursor.execute("UPDATE storage SET ischecked = 2, datecheck = '" + date + "', chekedby = " + str(id) + " WHERE id = " + str(idMedia))
    connection.commit()
    ...
    vk.method("messages.send",
                {"peer_id": idUser, "message": "Файл #" + str(idMedia) + " был отклонен.", "random_id": random.randint(1, 2147483647)})
