...
async def info(MessageSettings, user):
    ...
    try:
        Timetable = await showTimetable(group, 2, user, MessageSettings)
        if Timetable:
...
async def showTimetable(groupId, tomorrow=0, user=None, message=None):
    user_potok = user.potokLecture
    try:
        isNormal, response = await getResponse(groupId, user, message)
        if not isNormal:
            return response
        chetn = user.getChetn()
...
async def getResponse(groupId, user, MessageSettings):
    if user.own_shed:
