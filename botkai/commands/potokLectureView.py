...
def info():
    ...
    if res:
        cursor.execute("UPDATE users SET 'potokLecture' = 0 WHERE ID_VK = " + str(MessageSettings.getId()))
        vk.method("messages.send",
                        {"peer_id": id, "message": "Отображение потоковых лекций выключено", "random_id": random.randint(1, 2147483647)})
    else:
        cursor.execute("UPDATE users SET 'potokLecture' = 1 WHERE ID_VK = " + str(MessageSettings.getId()))
        vk.method("messages.send",
