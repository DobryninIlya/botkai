    ...
    try:
        ...
        if Timetable:
            vk.method("messages.send",
                        {"peer_id": id, "message": "Расписание на сегодня:\n" + Timetable + adv + task, "keyboard": keyboards.GetButtonTask(date), "random_id": random.randint(1, 2147483647)})
        else:
            vk.method("messages.send",
        ...
        vk.method("messages.send",
                    {"peer_id": id, "message": "Сегодня можно отдохнуть :]", "keyboard": keyboards.GetButtonTask(date), "random_id": random.randint(1, 2147483647)})
